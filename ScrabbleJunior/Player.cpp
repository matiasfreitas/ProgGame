...
void Player::play(Bag bag, Board board){
    ...
    while(indexHand == -1){
        ...
        if(board.getTiles(indexCol, indexRow).getValid()) {
            ...
            std::cout << "Tem certeza que consegue jogar nessa rodada? 1 para não, qualquer coisa para sim" << std::endl;
            std::cin >> choice;
            if(choice == 1){
                break;
            }
...
void Player::trade(Bag bag){
    ...
    for(int i = 0; i<2; i++){
        ...
        hand = hand.substr(0,indexTrade-1) + bag.tradeTile(std::string(1,hand[indexTrade]))+ hand.substr(indexTrade+1);
